...
import { iconPackConfig, requireIconPack, requireLocalIconPack } from './iconPackConfig';
...
function registerIconPacks(config: Array<{ prefix?: string; pack: string }>) {
	const iconPacks = config.map((iconPack) => ({
		...
		loader: () => {
			try {
				if (iconPack.pack.startsWith('@local/')) {
					// Handle local packages
					const localName = iconPack.pack.replace('@local/iconify-', '');
					const module = requireLocalIconPack(`./${localName}/icons.json`);
